...
<ul>
<li>변수 이름, 변수명, 식별자(identifier))<br> : 개발자가 지정하는, 메모리 공간에 저장된 값을 식별할 수 있는 고유한 이름. 식별자는 변수 값이 아닌 메모리 주소를 기억하고 있어야 한다.</li>
<li>변수 값<br> : 지정된 변수에 저장된 값이다.</li>
<li>할당(assignment, 대입, 저장)<br> : 특정 '변수 이름'에 '변수 값'을 저장하는 것이다.</li>
<li>참조(reference)<br> : 변수에 저장된 '변수 값'을 읽어 들이는 것이다.</li>
<li>선언(declaration)<br> : 변수를 생성하는 것을 말한다. 값을 저장하기 위한 메모리 공간을 확보하고, 변수 이름과 해당 메모리 공간의 주소를 연결하여 값을 저장할 수 있게 준비하는 것이다.</li>
</ul>
